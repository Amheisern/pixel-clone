            ...
            tutorialIntro2Next.onClick.AddListener(() =>
            {
                ...
                IEnumerator waitAndDisplayScanningTutorial()
                {
                    ...
                    scanningTutorialNext.onClick.AddListener(() =>
                    {
                        ...
                        {
                            if (page != poolPage || (DicePool.Instance.allDice.Count() == 0))// && DicePool.Instance.state != DiceManager.State.AddingDiscoveredDie))
                            {
                        ...
                        void onPageChanged(UIPage newPage, object context)
                        {
                            ...
                            //while (DicePool.Instance.state != DiceManager.State.Idle) yield return null;
                            ...
                            if (DicePool.Instance.allDice.Count() > 0)
                            {
                                ...
                                basicPreset.dieAssignments = DicePool.Instance.allDice.Select(
